...
const App = {
    provider: null, signer: null, contract: null, userAddress: null, wytDecimals: 18, paymentTokenDecimals: 18, allWorkOrders: [], elements: {},

    init() { this.cacheDOMElements(); this.addEventListeners(); console.log("App initialized."); this.checkExistingConnection(); },

    cacheDOMElements() {
        this.elements = {
            connectButton: document.getElementById('connectButton'), totalSupply: document.getElementById('totalSupply'), availableTokens: document.getElementById('availableTokens'),
            paymentBalance: document.getElementById('paymentBalance'), adminPanel: document.getElementById('adminPanel'), workOrderSearchContainer: document.getElementById('workOrderSearchContainer'),
            workOrderResults: document.getElementById('workOrderResults'), txHistoryTable: document.getElementById('txHistoryTable'), userBalance: document.getElementById('userBalance'),
            wytPrice: document.getElementById('wytPrice'), buyTab: document.getElementById('buyTab'), redeemTab: document.getElementById('redeemTab'), buyPanel: document.getElementById('buyPanel'),
            redeemPanel: document.getElementById('redeemPanel'), pUSDBalance: document.getElementById('pUSDBalance'), wytUserBalance: document.getElementById('wytUserBalance'),
            receiveAmount: document.getElementById('receiveAmount'), swapButton: document.getElementById('swapButton'), buyAmountInput: document.getElementById('buyAmount'),
            redeemAmountInput: document.getElementById('redeemAmount'), burnWytBalance: document.getElementById('burnWytBalance'), burnAmountInput: document.getElementById('burnAmount'),
            burnButton: document.getElementById('burnButton'), payoutOrderIdInput: document.getElementById('payoutOrderId'), payoutAmountInput: document.getElementById('payoutAmount'),
            payoutButton: document.getElementById('payoutButton'), collectedFees: document.getElementById('collectedFees'), mintAmountInput: document.getElementById('mintAmount'),
            mintDescriptionInput: document.getElementById('mintDescription'), mintButton: document.getElementById('mintButton'), fundIdInput: document.getElementById('fundId'),
            fundAmountInput: document.getElementById('fundAmount'), fundButton: document.getElementById('fundButton'), withdrawFeesButton: document.getElementById('withdrawFeesButton'),
            feeInput: document.getElementById('feeInput'), setFeeButton: document.getElementById('setFeeButton'), cancelIdInput: document.getElementById('cancelId'),
            cancelButton: document.getElementById('cancelButton'), exportPdfButton: document.getElementById('exportPdfButton')
        };
    },
    ...
    async checkExistingConnection() { if (window.ethereum) { try { const accounts = await window.ethereum.request({ method: 'eth_accounts' }); if (accounts.length > 0) await this.connectWallet(); } catch (error) { console.log('No existing connection found'); } } },
    disconnectWallet() { this.userAddress = null; this.provider = null; this.signer = null; this.contract = null; if (this.elements.connectButton) { this.elements.connectButton.textContent = 'Connect Wallet'; this.elements.connectButton.disabled = false; } if (this.elements.adminPanel) this.elements.adminPanel.classList.add('hidden'); },

    async checkAndSwitchNetwork() { if (!window.ethereum) throw new Error("Wallet not found"); try { const currentChainId = await window.ethereum.request({ method: 'eth_chainId' }); if (currentChainId === PLUME_MAINNET.chainId) return; try { await window.ethereum.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: PLUME_MAINNET.chainId }], }); } catch (switchError) { if (switchError.code === 4902) { await window.ethereum.request({ method: 'wallet_addEthereumChain', params: [PLUME_MAINNET], }); } else { throw switchError; } } } catch (error) { this.showNotification('Failed to switch to Plume Network. Please do it manually.', 'error'); throw error; } },
    async connectWallet() { if (!window.ethereum) return this.showNotification('Please install a Web3 wallet.', 'error'); try { this.showNotification('Connecting wallet...', 'info'); await this.checkAndSwitchNetwork(); const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' }); if (accounts.length === 0) throw new Error('No accounts found.'); this.provider = new ethers.providers.Web3Provider(window.ethereum); this.signer = this.provider.getSigner(); this.userAddress = await this.signer.getAddress(); this.contract = new ethers.Contract(contractAddress, contractABI, this.signer); const shortAddress = `${this.userAddress.slice(0, 6)}...${this.userAddress.slice(-4)}`; this.elements.connectButton.textContent = `Connected: ${shortAddress}`; this.elements.connectButton.disabled = true; this.showNotification('Wallet connected!', 'success'); await this.loadContractData(); this.switchTab('buy'); } catch (err) { this.showNotification(err.message || 'Connection failed.', 'error'); this.disconnectWallet(); } },
    async loadContractData() { try { this.showNotification('Loading contract data...', 'info'); await this.contract.name(); const paymentTokenAddress = await this.contract.paymentToken(); const paymentTokenContract = new ethers.Contract(paymentTokenAddress, tokenABI, this.provider); const [wytDecimals, paymentDecimals, totalSupply, available, paymentBalance, owner, userWytBalance, userPusdBalance, collectedFees] = await Promise.all([ this.contract.decimals(), paymentTokenContract.decimals(), this.contract.totalSupply(), this.contract.availableTokens(), this.contract.contractPaymentTokenBalance(), this.contract.owner(), this.contract.balanceOf(this.userAddress), paymentTokenContract.balanceOf(this.userAddress), this.contract.collectedFees() ]); this.wytDecimals = wytDecimals; this.paymentTokenDecimals = paymentDecimals; this.elements.totalSupply.innerHTML = this.formatTokenValue(totalSupply, this.wytDecimals); this.elements.availableTokens.innerHTML = this.formatTokenValue(available, this.wytDecimals); this.elements.paymentBalance.innerHTML = this.formatTokenValue(paymentBalance, this.paymentTokenDecimals); this.elements.userBalance.innerHTML = this.formatTokenValue(userWytBalance, this.wytDecimals); this.elements.collectedFees.innerHTML = this.formatTokenValue(collectedFees, this.paymentTokenDecimals); this.elements.wytUserBalance.textContent = this.formatTokenValue(userWytBalance, this.wytDecimals); this.elements.pUSDBalance.textContent = this.formatTokenValue(userPusdBalance, this.paymentTokenDecimals); this.elements.burnWytBalance.textContent = this.formatTokenValue(userWytBalance, this.wytDecimals); if (!totalSupply.isZero()) { const price = paymentBalance.mul(ethers.utils.parseUnits("1", this.wytDecimals)).div(totalSupply); this.elements.wytPrice.innerHTML = this.formatTokenValue(price, this.paymentTokenDecimals); } else { this.elements.wytPrice.innerHTML = '0.00'; } if (owner.toLowerCase() === this.userAddress.toLowerCase()) { this.elements.adminPanel.classList.remove('hidden'); } this.renderSearchBar(); await this.renderWorkOrders(); await this.renderTransactionHistory(); this.updateReceiveAmount(); this.showNotification('Contract data loaded.', 'success'); } catch (error) { this.showNotification('Failed to load contract data.', 'error'); } },
    async getPaymentTokenContract() { const paymentTokenAddress = await this.contract.paymentToken(); return new ethers.Contract(paymentTokenAddress, tokenABI, this.signer); },
    async executeSwap() { const isBuy = !this.elements.buyPanel.classList.contains('hidden'); if (isBuy) { await this.handleTransaction(this.elements.swapButton, async () => { const amount = this.elements.buyAmountInput.value; if (!amount || parseFloat(amount) <= 0) throw new Error("Please enter a valid amount."); const parsedAmount = ethers.utils.parseUnits(amount, this.paymentTokenDecimals); const paymentToken = await this.getPaymentTokenContract(); this.showNotification('Approving spend...', 'info'); const approveTx = await paymentToken.approve(contractAddress, parsedAmount); await approveTx.wait(); this.showNotification('Approval successful! Now buying...', 'info'); const buyTx = await this.contract.buyTokens(parsedAmount); await buyTx.wait(); this.elements.buyAmountInput.value = ''; return 'WYT purchased successfully!'; }); } else { await this.handleTransaction(this.elements.swapButton, async () => { const amount = this.elements.redeemAmountInput.value; if (!amount || parseFloat(amount) <= 0) throw new Error("Please enter a valid amount."); const parsedAmount = ethers.utils.parseUnits(amount, this.wytDecimals); const tx = await this.contract.redeemTokens(parsedAmount); await tx.wait(); this.elements.redeemAmountInput.value = ''; return 'WYT redeemed successfully!'; }); } },
    switchTab(tab) { const isBuy = tab === 'buy'; this.elements.buyTab.classList.toggle('active', isBuy); this.elements.redeemTab.classList.toggle('active', !isBuy); this.elements.buyPanel.classList.toggle('hidden', !isBuy); this.elements.redeemPanel.classList.toggle('hidden', isBuy); this.updateReceiveAmount(); },
    updateReceiveAmount() { const priceText = this.elements.wytPrice.textContent.replace(/,/g, ''); const price = parseFloat(priceText); if (isNaN(price)) { this.elements.swapButton.textContent = 'Price Unavailable'; this.elements.swapButton.disabled = true; return; } const isBuy = !this.elements.buyPanel.classList.contains('hidden'); let receiveAmount = 0; let buttonText = 'Enter an amount'; let disabled = true; let amountInput = isBuy ? this.elements.buyAmountInput : this.elements.redeemAmountInput; const amount = parseFloat(amountInput.value); if (!isNaN(amount) && amount > 0) { if (isBuy) { if (price > 0) receiveAmount = amount / price; buttonText = 'Buy WYT'; } else { receiveAmount = amount * price; buttonText = 'Redeem WYT'; } disabled = false; } this.elements.receiveAmount.textContent = receiveAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 4 }); this.elements.swapButton.textContent = buttonText; this.elements.swapButton.disabled = disabled; },
    async mintWorkOrder() { await this.handleTransaction(this.elements.mintButton, async () => { const grossYield = this.elements.mintAmountInput.value; const desc = this.elements.mintDescriptionInput.value; if (!grossYield || !desc) throw new Error("Gross yield and description are required."); const parsedGrossYield = ethers.utils.parseUnits(grossYield, this.paymentTokenDecimals); const tx = await this.contract.mintFromWorkOrder(parsedGrossYield, desc); await tx.wait(); this.elements.mintAmountInput.value = ''; this.elements.mintDescriptionInput.value = ''; return 'Work order minted!'; }); },
    async fundWorkOrder() { await this.handleTransaction(this.elements.fundButton, async () => { const id = this.elements.fundIdInput.value; const amount = this.elements.fundAmountInput.value; if (!id || !amount) throw new Error("Work Order ID and amount are required."); const parsedAmount = ethers.utils.parseUnits(amount, this.paymentTokenDecimals); const paymentToken = await this.getPaymentTokenContract(); const approveTx = await paymentToken.approve(contractAddress, parsedAmount); this.showNotification('Approving spend... please wait.', 'info'); await approveTx.wait(); this.showNotification('Approval successful! Now funding...', 'info'); const tx = await this.contract.fundWorkOrder(id, parsedAmount); await tx.wait(); this.elements.fundIdInput.value = ''; this.elements.fundAmountInput.value = ''; return 'Work order funded!'; }); },
    async withdrawFees() { await this.handleTransaction(this.elements.withdrawFeesButton, async () => { const tx = await this.contract.withdrawFees(); await tx.wait(); return 'Fees withdrawn successfully!'; }); },
    async setRedemptionFee() { await this.handleTransaction(this.elements.setFeeButton, async () => { const newFee = parseInt(this.elements.feeInput.value); if (isNaN(newFee) || newFee < 0 || newFee > 20) { throw new Error("Fee must be a number between 0 and 20."); } const tx = await this.contract.setRedemptionFee(newFee); await tx.wait(); this.elements.feeInput.value = ''; return 'Redemption fee updated!'; }); },
    async cancelWorkOrder() { await this.handleTransaction(this.elements.cancelButton, async () => { const id = parseInt(this.elements.cancelIdInput.value); if (isNaN(id) || id <= 0) throw new Error("Please enter a valid Work Order ID."); const tx = await this.contract.cancelWorkOrder(id); await tx.wait(); this.elements.cancelIdInput.value = ''; return 'Work order cancelled!'; }); },
    async burnTokens() { await this.handleTransaction(this.elements.burnButton, async () => { const amount = this.elements.burnAmountInput.value; if (!amount || parseFloat(amount) <= 0) throw new Error("Please enter a valid amount to burn."); const parsedAmount = ethers.utils.parseUnits(amount, this.wytDecimals); const tx = await this.contract.burn(parsedAmount); await tx.wait(); this.elements.burnAmountInput.value = ''; return 'Tokens burned successfully!'; }); },
    async payoutWorkOrder() { await this.handleTransaction(this.elements.payoutButton, async () => { const id = this.elements.payoutOrderIdInput.value; const amount = this.elements.payoutAmountInput.value; if (!id || !amount || parseFloat(amount) <= 0) { throw new Error("Please enter a valid Work Order ID and payout amount."); } const parsedAmount = ethers.utils.parseUnits(amount, this.paymentTokenDecimals); const paymentToken = await this.getPaymentTokenContract(); this.showNotification('Approving payout... please wait.', 'info'); const approveTx = await paymentToken.approve(contractAddress, parsedAmount); await approveTx.wait(); this.showNotification('Approval successful! Submitting payout...', 'info'); const tx = await this.contract.payoutWorkOrder(id, parsedAmount); await tx.wait(); this.elements.payoutOrderIdInput.value = ''; this.elements.payoutAmountInput.value = ''; return 'Work order payout successful!'; }); },

    renderSearchBar() { if (!this.elements.workOrderSearchContainer) return; this.elements.workOrderSearchContainer.innerHTML = `<div class="wo-search-container"><input type="text" id="workOrderSearchInput" placeholder="Search by ID or Description..."></div>`; },
    async renderWorkOrders() { if (!this.elements.workOrderResults) return; this.elements.workOrderResults.innerHTML = '<p>Loading work orders...</p>'; try { const nextId = await this.contract.nextWorkOrderId(); const promises = []; for (let i = 1; i < nextId; i++) promises.push(this.contract.workOrders(i)); this.allWorkOrders = (await Promise.all(promises)).filter(wo => wo.id > 0); this.displayWorkOrders(this.allWorkOrders); } catch (err) { this.elements.workOrderResults.innerHTML = '<p class="text-red-500">Error loading work orders.</p>'; } },
    displayWorkOrders(orders, isSearchResult = false) { if (!this.elements.workOrderResults) return; if (orders.length === 0) { this.elements.workOrderResults.innerHTML = `<p>${isSearchResult ? 'No matching work order found.' : 'No work orders found.'}</p>`; return; } if (isSearchResult) { const tableHtml = `<table><thead><tr><th>ID</th><th>Gross Yield</th><th>Reserve</th><th>Issued</th><th>Active</th><th>Paid</th><th>Description</th><th>Created</th></tr></thead><tbody>${orders.map(wo => `<tr><td>${wo.id}</td><td>${this.formatTokenValue(wo.grossYield, this.paymentTokenDecimals)}</td><td>${this.formatTokenValue(wo.reserveAmount, this.paymentTokenDecimals)}</td><td>${this.formatTokenValue(wo.tokensIssued, this.wytDecimals)}</td><td>${wo.isActive ? '✅' : '❌'}</td><td>${wo.isPaid ? '✅' : '❌'}</td><td>${wo.description}</td><td>${new Date(wo.createdAt * 1000).toLocaleDateString()}</td></tr>`).join('')}</tbody></table>`; this.elements.workOrderResults.innerHTML = tableHtml; return; } const groupedOrders = orders.reduce((acc, wo) => { const date = new Date(wo.createdAt * 1000); const key = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}`; if (!acc[key]) acc[key] = []; acc[key].push(wo); return acc; }, {}); const sortedMonths = Object.keys(groupedOrders).sort().reverse(); const tabsHtml = sortedMonths.map((monthKey, index) => { const [year, monthNum] = monthKey.split('-'); const monthName = new Date(year, monthNum - 1).toLocaleString('default', { month: 'long', year: 'numeric' }); return `<button class="wo-tab-btn ${index === 0 ? 'active' : ''}" data-month="${monthKey}">${monthName}</button>`; }).join(''); const contentHtml = sortedMonths.map((monthKey, index) => { const ordersForMonth = groupedOrders[monthKey]; const tableHeader = `<thead><tr><th>ID</th><th>Gross Yield</th><th>Reserve</th><th>Issued</th><th>Active</th><th>Paid</th><th>Description</th><th>Created</th></tr></thead>`; const tableBody = `<tbody>${ordersForMonth.map(wo => `<tr><td>${wo.id}</td><td>${this.formatTokenValue(wo.grossYield, this.paymentTokenDecimals)}</td><td>${this.formatTokenValue(wo.reserveAmount, this.paymentTokenDecimals)}</td><td>${this.formatTokenValue(wo.tokensIssued, this.wytDecimals)}</td><td>${wo.isActive ? '✅' : '❌'}</td><td>${wo.isPaid ? '✅' : '❌'}</td><td>${wo.description}</td><td>${new Date(wo.createdAt * 1000).toLocaleDateString()}</td></tr>`).join('')}</tbody>`; return `<div id="wo-content-${monthKey}" class="wo-content-panel ${index > 0 ? 'hidden' : ''}"><table>${tableHeader}${tableBody}</table></div>`; }).join(''); this.elements.workOrderResults.innerHTML = `<div class="wo-tabs">${tabsHtml}</div><div class="wo-content">${contentHtml}</div>`; },
