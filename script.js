...
const App = {
    ...
    cacheDOMElements() {
        this.elements = {
            ...
            // MODIFICATION: Cached new container IDs from HTML
    ...
    addEventListeners() {
        ...
        // MODIFICATION: Attached listeners to the correct, separated containers
    ...
    async checkAndSwitchNetwork() {
        ...
        // ... (code for this function is correct as is)
    },
    ...
    async loadContractData() {
        try {
            ...
            // MODIFICATION: Added call to render the search bar once
    ...
    // NEW: Renders the search bar just once.
    renderSearchBar() {
    ...
    // MODIFIED: Fetches data and calls the display function.
    ...
    // NEW: Handles all rendering logic for the results area.
    displayWorkOrders(orders, isSearchResult = false) {
        ...
        if (orders.length === 0) {
            ...
            this.elements.workOrderResults.innerHTML = `<p>${message}</p>`;
            return;
        }
...
const styles = `
    .notification {
        ...
        max-width: 300px; word-wrap: break-word; animation: slideIn 0.3s ease-out;
    }
    ...
    .spinner {
        ...
        border: 2px solid #ffffff30; border-top: 2px solid #ffffff;
    ...
    #workOrderSearchInput {
        width: 100%; padding: 0.5rem 0.75rem; background-color: #2d3748;
        border: 1px solid #4a5568; border-radius: 0.375rem; color: white;
    }
    #workOrderSearchInput::placeholder { color: #a0aec0; }
    .wo-tabs {
        display: flex; flex-wrap: wrap; border-bottom: 2px solid #4a5568;
        margin-bottom: 1rem;
    }
    .wo-tab-btn {
        padding: 0.5rem 1rem; border: none; background-color: transparent;
        color: #a0aec0; cursor: pointer; font-weight: 600;
